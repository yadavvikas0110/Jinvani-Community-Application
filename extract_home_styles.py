...
try:
    ...
    flows_to_check = ["Home Page", "Community Feed, Posts and Blogs Flow"]
    all_children = data["nodes"]["592:15086"]["document"]["children"]
    
    summary = {}
    for flow in all_children:
        # Some are single nodes named "Home Page", others are groups.
        if flow.get("name") in flows_to_check:
            ...
            if flow.get("name") in summary:
                 # merge if multiple home pages exist
                 summary[flow.get("name")].update(flow_summary)
            else:
                 summary[flow.get("name")] = flow_summary
                
    with open('home_figma_summary.json', 'w') as out:
        json.dump(summary, out, indent=2)
    print("Home summary generated in home_figma_summary.json")
